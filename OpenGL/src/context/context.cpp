...
bool Context::Init() {
    ...
    m_simpleProgram = Program::Create("src/shader/simple.vs", "src/shader/simple.fs");
    if (!m_simpleProgram)
        return false;

    m_program = Program::Create("src/shader/lighting.vs", "src/shader/lighting.fs");
    if (!m_program)
        return false;
    ...
    m_box2Material->diffuse = Texture::CreateFromImage(
        Image::Load("resource/tex/container2.png").get());
    m_box2Material->specular = Texture::CreateFromImage(
        Image::Load("resource/tex/container2_specular.png").get());
