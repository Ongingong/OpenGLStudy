...
bool Context::Init()
{
    ...
    // Load Texture
    m_material.diffuse = Texture::CreateFromImage( Image::Load("resource/tex/container2.png").get());
    m_material.specular = Texture::CreateFromImage( Image::Load("resource/tex/container2_specular.png").get());
    // Create program
    m_simpleProgram = Program::Create(vs_simple_path, fs_simple_path);
    if (!m_simpleProgram)
        return false;

    m_program = Program::Create(vs_path, fs_path);
    if (!m_program)
        return false;
...
void Context::Render()
{
    if (ImGui::Begin("ui window")) {
        if (ImGui::CollapsingHeader("light", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (ImGui::DragFloat("l.distance", &m_light.distance, 1.0f, 1.0f, 256.0f)) {
                if (m_light.distance < 1.0f) m_light.distance = 1.0f;
                if (m_light.distance > 256.0f) m_light.distance = 256.0f;
            }
            ImGui::DragFloat2("l.cutoff", glm::value_ptr(m_light.cutoff), 1.0f, 1.0f, 180.0f);
